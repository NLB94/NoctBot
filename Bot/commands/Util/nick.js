...
module.exports.run = functions.run = async (client, message, args) => {
    ...
    try {
        ...
        if (user.id !== message.author.id && (!message.member.permissions.has('MANAGE_NICKNAMES') || message.member.roles.highest.position < member)) return message.channel.send({ embeds: [{ description: `${x_mark} You don't have enough permissions to change this user nickname !` }]})
        ...
        if (user.id !== message.author.id && (!message.member.permissions.has('MANAGE_NICKNAMES') || message.member.roles.highest.position < member.roles.highest.position)) return message.channel.send({ embeds: [{ description: `${x_mark} You don't have enough permissions to change this user nickname !` }]})
        ...
        }).catch(err => message.channel.send({ embeds: [{ title: 'Error !', description: `Cannot change user's nickname ! \nThe error is : \`${err}\`` }]}))
    } catch (e) {
        // console.log(e);
        message.channel.send({ embeds: [{ title: 'Error !', description: `Cannot change user's nickname ! \nMissing permissions !` }]})
    }
