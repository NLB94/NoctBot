...
module.exports.run = functions.run = async (client, message, args, settings, userInfo, strings) => {
  ...
  const embed = new MessageEmbed()
    ...
    .addFields({
        name: `► Prefix :`,
        value: settings.general.prefix !== '~' ? settings.general.prefix : "~",
        inline: true
      }, {
        name: '► Logs Channel :',
        value: settings.general.logs !== 'logs' ? `<#${settings.general.logs}>` : `Unspecified ! Type \`${settings.general.prefix}config logs #channel\`!`,
        inline: true
      }, {
        name: '\u200b',
        value: '\u200b',
