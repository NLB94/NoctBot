...
module.exports = functions.client = async client => {
  ...
  console.log(
    /*chalk.bgWhite.black(*/
    `${client.user.tag} est prêt alhamduliLlah ! ${guilds.length} serveurs, ${client.users.cache.size} membres!` //)
  );

  let i = 0,
    presence = ['PLAYING', 'WATCHING'],
    j = 0;

  setInterval(() => {
      let activities = ["~help", `${client.users.cache.size} members in ${guilds.length} servers !`];
