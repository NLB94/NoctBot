        ...
        giveaway.channel.send(embed).then(async msg => {
            ...
            setTimeout(async () => {
                ...
                for (let w of winners) {
                    if (!w || w == undefined || w == '') continue;
                    wins.push(w)
                }
                if (reactions.cache.size < giveaway.winnerCount) {
                    ...
                    msg.edit(endEmbed);
                    await msg.channel.send(`Congratulations ${wins.join(", ")} ! You won \`${giveaway.price}\` !`)
                    client.endGiveaway(msg.guild, giveaway.id)
                }
            }, ms(giveaway.time));
        })
    } catch (e) {
