...
module.exports.run = functions.run = async (client, message, args, settings, userInfo) => {
          ...
          .setDescription(`[Add Me](${client.botGuild.utilInviteLink}) | [Support Server](${client.botGuild.supportInvite})`)
          .addFields(
              { name: `► Prefix :`, value: settings.general.utilPrefix !== 'u~' ? settings.general.utilPrefix : 'u~', inline: true},
              { name: '► Logs Channel :', value: settings.general.logs !== 'logs' ? `<#${settings.general.logs}>` : `Unspecified ! Type \`${settings.general.utilPrefix}config logs #channel\`!`, inline: true},
              { name: '\u200b', value: '\u200b', inline: true},
              // { name: `► Welcome Enabled :`, value: settings.welcomeAndLeave.welcome.enable ? 'Yes' : `No.`/* For enable, type \`${settings.general.utilPrefix}enable welcome\``*/, inline: true},
              // { name: `► Leave Enabled :`, value: settings.welcomeAndLeave.leave.enable ? 'Yes' : `No.` /*For enable, type \`${settings.general.utilPrefix}enable leave\``*/, inline: true},
            //⌠ ⌡ ►
          )
          .setTimestamp()
