  ...
  if (!args.length) {
    if (message.guild && (message.guild.me.permissions.has(1074004032) || message.guild.me.permissions.has('ADMINISTRATOR'))) {
      const embed = new MessageEmbed()
        .setColor("#000000")
        .setAuthor("Noct", client.user.avatarURL(), `${client.botGuild.utilInviteLink}`)
      ...
      const newEmbed = new MessageEmbed()
        ...
        .setTitle("Bot Commands")
        .setURL(`${client.botGuild.supportInvite}`)
        .setDescription(`My prefix in this server is ***\`${settings == undefined ? 'u~' : settings.general.utilPrefix}\`*** \nIf you need more informations about commands, type ${settings == undefined ? 'u~' : settings.general.utilPrefix}help <command | category>!`)
        .setTimestamp()
    ...
    if (cat) {
      ...
      const embed = new MessageEmbed()
        ...
        .addField("Usage :", command.help.usage ? `${settings == undefined ? 'u~' : settings.general.utilPrefix}${command.help.name} ${command.help.usage}` : `${settings == undefined ? 'u~' : settings.general.utilPrefix}${command.help.name}`, true)
    ...
    if (parseInt(args[0]) > 9 || parseInt(args[0] < 1)) return message.channel.send({
      embed: {
        description: `Correct usage : ${settings == undefined ? 'u~' : settings.general.utilPrefix}help ${module.exports.help.usage}`
      }
    });
    else {
      const categorys = []
