  ...
  if (!args.length) {
    if (message.guild && (message.guild.me.permissions.has(1074004032) || message.guild.me.permissions.has('ADMINISTRATOR'))) {
      const embed = new MessageEmbed()
        .setColor("#000000")
        .setAuthor("Noct", client.user.avatarURL(), `${client.botGuild.protectInviteLink}`)
      ...
      const newEmbed = new MessageEmbed()
        ...
        .setTitle("Bot Commands")
        .setURL(`${client.botGuild.supportInvite}`)
        .setDescription(`My prefix in this server is ***\`${settings == undefined ? 'p~' : settings.general.protectPrefix}\`*** \nIf you need more informations about commands, type ${settings == undefined ? 'p~' : settings.general.protectPrefix}help <command | category>!`)
        .setTimestamp()
    ...
    if (cat) {
      ...
      const embed = new MessageEmbed()
        ...
        .addField("Usage :", command.help.usage ? `${settings == undefined ? 'p~' : settings.general.protectPrefix}${command.help.name} ${command.help.usage}` : `${settings == undefined ? 'p~' : settings.general.protectPrefix}${command.help.name}`, true)
    ...
    if (parseInt(args[0]) > 9 || parseInt(args[0] < 1)) return message.channel.send({
      embed: {
        description: `Correct usage : ${settings == undefined ? 'p~' : settings.general.protectPrefix}help ${module.exports.help.usage}`
      }
    });
    else {
      const categorys = []
      ...
      const embed = new MessageEmbed()
        .setTitle(category)
        .setAuthor(client.user.tag, client.user.avatarURL(), `${client.botGuild.protectInviteLink}`)
