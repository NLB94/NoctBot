...
module.exports.run = functions.run = async (client, message, args, settings, userInfo)  => {
    ...
    switch (getSetting) {
        ...
        case "ticket": {
            message.channel.send({embed: {title: 'Server Settings', description: `${x_mark}Ticket system is not ready yet...`}})
        ...
        case "welcome": {
            if (settings.welcomeAndLeave.welcome.enable) return message.channel.send({embed: {title: 'Server Settings', description: `${x_mark}Welcome system is already enable !`}});
            await client.updateGuild(message.guild, {
                "welcomeAndLeave.welcome.enable": true
            });
            message.channel.send({embed: {title: 'Server Settings', description: `${check_mark}Successfully enabled welcome system !Type ${settings.general.protectPrefix}welcome keys for more !`}});
            break;
        }
        case "leave": {
            if (settings.welcomeAndLeave.leave.enable) return message.channel.send({embed: {description: `${x_mark}Leave system is already enable !`}});
            await client.updateGuild(message.guild, {
                "welcomeAndLeave.leave.enable": true
            });
            message.channel.send({embed: {title: 'Server Settings', description: `${check_mark}Successfully enabled leave system \nType ${settings.general.protectPrefix}leave keys for more !`}});
            break;
        }
    }
};
