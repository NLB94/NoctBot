...
module.exports.run = functions.run = async (client, message, args, settings, userInfo) => {
          ...
          .setURL(`${client.botGuild.protectInviteLink}`)
          .setThumbnail(client.user.displayAvatarURL())
          .setDescription(`[Add Me](${client.botGuild.protectInviteLink}) | [Support Server](${client.botGuild.supportInvite})`)
          .addFields(
              { name: `► Prefix :`, value: settings.general.protectPrefix !== 'p~' ? settings.general.protectPrefix : 'p~', inline: true},
              { name: '► Logs Channel :', value: settings.general.logs !== 'logs' ? `<#${settings.general.logs}>` : `Unspecified ! Type \`${settings.general.protectPrefix}config logs #channel\`!`, inline: true},
              { name: '\u200b', value: '\u200b', inline: true},
              // { name: `► Welcome Enabled :`, value: settings.welcomeAndLeave.welcome.enable ? 'Yes' : `No.`/* For enable, type \`${settings.general.protectPrefix}enable welcome\``*/, inline: true},
              // { name: `► Leave Enabled :`, value: settings.welcomeAndLeave.leave.enable ? 'Yes' : `No.` /*For enable, type \`${settings.general.protectPrefix}enable leave\``*/, inline: true},
            //⌠ ⌡ ►
          )
          .setTimestamp()
